         ...
         load_data=importdata('inputfile_16.txt')% loading all data (characters) into array load_data
                %load data will be cell array(1*1)
                    ...
                    ksrr=load_data(:);% accessing all elements
                        ...
                        var=ismember(ksrr,text_char); %checking characters in text file
                        length_char=sum(var)%if any alphabet of text file matches with lowercase array,then ismember returns 1
                         k=1;      
                        while k<=prod(size(text_char))% loop for checking each and every alphabet of text file with lower case
